...
from math import atan2, hypot, floor, pi
from numpy import clip

class SkyImageConverter(CubeMapImageProcessor):
  getimageError = getimageError

  def __init__ (self, imgIn, imgOut, progress_window, create_process, percentage):
    super().__init__(progress_window, create_process, percentage)
    self.imgIn = imgIn
    self.imgOut = imgOut
  ...
  def convertBack(self, pv):
    ...
    for i in range(outSize[0]):
      ...
      current_percent = self.CurrentProgress(float(pv), current_percent)
      for j in rng:
        ...
        vf = ( configs['output']['curvature']*edge * (pi/1.869 - phi)/pi)
  ...
  def OutputValues(self, inSize):
    # Set the progress_bar parameters based on input image size dynamically and proportionally
    edge = inSize[0] / 4
    blend_width = int(edge * configs['output']['edge_blend'])

    if inSize[0] >= 3840 or inSize[1] >= 2160: pv, correct_position = 0.010, 6
    elif inSize[0] >= 2048 or inSize[1] >= 1080: pv, correct_position = 0.0225, 4
